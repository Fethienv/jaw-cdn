...
class TermMeta {
	...
	public function scheduleImport() {
		if ( aioseo()->helpers->scheduleSingleAction( aioseo()->importExport->seoPress->termActionName, 0 ) ) {
			if ( ! aioseo()->transients->get( 'import_term_meta_seopress' ) ) {
				aioseo()->transients->update( 'import_term_meta_seopress', time(), WEEK_IN_SECONDS );
			}
		}
	}
	...
	public function importTermMeta() {
		...
		$timeStarted      = gmdate( 'Y-m-d H:i:s', aioseo()->transients->get( 'import_term_meta_seopress' ) );
		...
		if ( ! $terms || ! count( $terms ) ) {
			aioseo()->transients->delete( 'import_term_meta_seopress' );
			return;
		}
