...
class MonsterInsights_AMP_Parser extends AMP_Base_Sanitizer {
	...
	public function sanitize() {
		...
		$this->options    = array(
			...
			'internal_as_outbound_label' => "outbound-link-" . $internal_label,
		);
	...
	private function parse_href( $node ) {
		...
		if ( ! empty( $url['path'] ) && ! empty( $this->options['internal_as_outbound'] ) && ! empty( $this->options['internal_as_outbound_label'] ) ) {
